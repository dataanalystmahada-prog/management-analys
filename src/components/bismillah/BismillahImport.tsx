...
export function BismillahImport({ onBack, onImportComplete }: Props) {
  ...
  const processData = () => {
    ...
    rawData.forEach((row, index) => {
      ...
      dbColumns.forEach(dbCol => {
        ...
        // Validation
        if (['qty', 'potensi_omset'].includes(dbCol) && val !== null) {
        ...
        if (['tanggal', 't_date', 'p_date'].includes(dbCol) && val !== null) {
  ...
  const checkGlobalDuplicates = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('bismillah')
        .select('nama_lengkap, perusahaan, tanggal');
      
      if (error) throw error;

      const existingSet = new Set(
        data.map(d => `${d.nama_lengkap || ''}|${d.perusahaan || ''}|${d.tanggal || ''}`.toLowerCase())
      );
      ...
      mappedData.valid.forEach(row => {
        const key = `${row.nama_lengkap || ''}|${row.perusahaan || ''}|${row.tanggal || ''}`.toLowerCase();
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      ...
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-100">
        <h2 className="text-xl font-semibold text-gray-900 flex items-center">
          <Upload className="mr-2 text-blue-600" /> Import Master Bismillah
        </h2>
        <button 
      ...
      {step === 'mapping' && (
        <div className="space-y-6">
          <div className="bg-blue-50 p-4 rounded-lg text-sm text-blue-800">
            Mapping kolom dari file CSV Anda ({file?.name}) ke database Bismillah.
          </div>
      ...
      {step === 'preview2' && (
        <div className="space-y-6">
          <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg text-sm text-yellow-800">
            <strong>Preview Tahap 2 (Cek Database Global):</strong> Mengecek duplikasi dengan data yang <strong>sudah ada di Supabase</strong> berdasarkan <strong>Nama Lengkap + Perusahaan + Tanggal</strong>.
          </div>
